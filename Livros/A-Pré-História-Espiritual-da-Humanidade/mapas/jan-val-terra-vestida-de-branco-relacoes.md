---
title: "Mapa de relações — Terra vestida de branco"
nav_exclude: true
---
...
```mermaid
flowchart TD
    JVE[Jan Val Ellam — fonte] --> BJ[Brahma / Javé]
    BJ --> D[Daksha / Daxa]
    D --> K[Kadru / Cadru]
    D --> DI[Diti]
    D --> AD[Aditi]
    D --> VI[Vinata]
    K --> KA[Kashyapa]
    K --> N[Nagas e outros grupos serpentinos/draconoides]
    N --> M[Conselho dos Maroragas]
    M --> T[Tirthankaras]
    T --> MA[Mahavira]

    Y[Yggdrasil — nove mundos/faixas na leitura de Jan] --> MG[Midgard = Marte, segundo Jan]
    Y --> O[Odin]
    Y --> F[Frigg / Freyr]
    Y --> AV[Aesir / Vanir]
    TR[Trimurti] --> C[Conflito com Yggdrasil]
    C --> R[Ragnarök — ~300 Ma na cronologia de Jan]
    R --> TE[Transferência de seres para a Terra]

    K --> EV[Evolução dos vertebrados — ~375 Ma]
    TK[Tiktaalik — fóssil científico ~375 Ma] --> EV
    TE --> EV

    EV --> PC[Preparação biológica da consciência]
    PC --> PE[Pandora / Eva — despertar da consciência em outra etapa da narrativa]
```
